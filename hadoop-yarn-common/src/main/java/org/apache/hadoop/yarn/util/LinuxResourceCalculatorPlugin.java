...
public class LinuxResourceCalculatorPlugin extends ResourceCalculatorPlugin {
  ...
  //JISHA 
  private static final String PROCFS_DISK_STATS = "/proc/diskstats";
  ...
  private String procfsDiskStatFile;
  private static final int UNAVAILABLE = -1;
  private long diskLastSampleTime = UNAVAILABLE;
  private float avgCpuUsage = UNAVAILABLE;
  private float avgIOUtil = UNAVAILABLE;
  private long sampleDiskStatTime = UNAVAILABLE;
  private long lastSampleDiskStatTime = UNAVAILABLE;
  private long lastDiskStat = UNAVAILABLE;
   ...
   * 
   */
	public void readProcDiskStatFile() {
		// Read "/proc/diskstats" file
		BufferedReader in = null;
		FileReader fReader = null;
		sampleDiskStatTime = getCurrentTime();
		try {
			fReader = new FileReader(procfsDiskStatFile);
			in = new BufferedReader(fReader);
		} catch (FileNotFoundException f) {
			return;
		}
		...
		try {
			...
			while (str != null) {
				...
				if (str.matches(sda)) {
					LOG.info("JISHA reading procstat line : " + str);
					String[] data = str.split("[ ]");
					diskStat = Integer.parseInt(data[data.length - 2]);
					LOG.info("JISHA diskStat : "+ diskStat);
					if (lastSampleDiskStatTime > 0) {
						LOG.info("JISHA : lastSampleDiskStatTime > 0: " + lastSampleDiskStatTime);
						double diffStat = diskStat - lastDiskStat;
						LOG.info("JISHA : diffStat : " + diffStat);
						double diffTime = sampleDiskStatTime
								- lastSampleDiskStatTime;
						LOG.info("JISHA : diffTime : " + diffTime);
						busy = (int) Math.round((diffStat) * 100 / (diffTime));
						if (busy > 100) {
							busy = 100;
						}// end of inner if loop
						LOG.info("JISHA : BUSY :" + busy);
					} else { // Set busy to 0 initially.
						LOG.info("JISHA : initialising BUSY :" + busy);
						busy = 0;
					}
					avgIOUtil = busy;
					break;
			...
			lastSampleDiskStatTime = sampleDiskStatTime;
			LOG.info("JISHA : Average IO util from ResourceCalculator : " + avgIOUtil);
		} catch (IOException io) {
			LOG.warn("Error reading the stream " + io);
		} finally {
			// Close the streams
	...
	private void readDiskStats() {
		long diskSampleTime = getCurrentTime();
		if (diskLastSampleTime == UNAVAILABLE
				|| diskLastSampleTime > diskSampleTime
				|| diskSampleTime > diskLastSampleTime + 999) { // update every
																// sec

			Runtime rt = Runtime.getRuntime();
			Process p = null;
			String sda = ".*\\bsda\\b.*";
			String avgCpu = ".*\\bavg-cpu:\\b.*";
			try {
				p = rt.exec("iostat -kx 1 2 sda");
			...
			int count = 0;
			try {
				LOG.info("JISHA : reading iostat data...");
				List<Float> cpu_data = new ArrayList<Float>();
				while (sc.hasNext()) {
					String line = sc.nextLine();
					LOG.info("JISHA : line from iostat = "+ line);
					if (line.contains("avg-cpu:")) {
						line = sc.nextLine();
						info = new Scanner(line);
						for (int i = 0; i < 5; i++)
							info.next(); // skip all unwanted values
						cpu_data.add(100 - Float.valueOf(info.next()));
						LOG.info(" CPU Usage from IOStat = "
								+ avgCpuUsage);

					}
					if (line.matches(sda)) { // TODO: take the partition that
												// hadoop is actually running on
						count++;
						if (count < 2) {
							continue;
						}
				...
				OptionalDouble average = cpu_data
			            .stream()
			            .mapToDouble(a -> a)
			            .average();
				avgCpuUsage = average.isPresent() ? (float)average.getAsDouble() : 0f; 
				LOG.info("Average CPU Usage from resource calculator : " + avgCpuUsage);
				for (int i = 0; i < 13; i++)
					info.next(); // skip device, tps, kb_read/s, kb_wrtn/s

				// cumulativeDiskReadKiloBytes =Long.parseLong(info.next());
				// cumulativeDiskWriteKiloBytes = Long.parseLong(info.next());

//				avgIOUtil = Float.valueOf(info.next());
//
//				
//				LOG.info("Average IO Usage from iostat : " + avgIOUtil);
  ...
  public static void main(String[] args) {
	...
	System.out.println("CPU usage from IO % : " + plugin.getAvgCpuUsage());
	System.out.println("IO Utilization % : " + plugin.getAvgIOUtil());
  }
}
