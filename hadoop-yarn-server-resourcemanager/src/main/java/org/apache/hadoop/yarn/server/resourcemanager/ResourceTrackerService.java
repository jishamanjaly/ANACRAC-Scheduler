...
public class ResourceTrackerService extends AbstractService implements
    ...
    if (masterContainer.getId().equals(containerStatus.getContainerId())
      ...
      ContainerStatus status =
          ContainerStatus.newInstance(containerStatus.getContainerId(),
            ...
            containerStatus.getContainerExitStatus(), containerStatus.getCpuUsage(), containerStatus.getMemoryUsage()); // JISHA
  ...
  public NodeHeartbeatResponse nodeHeartbeat(NodeHeartbeatRequest request)
    ...
    LOG.info("JISHA : Received heartbeat from Node : "+ remoteNodeStatus.getNodeId());
    
    LOG.info("JISHA : Received heartbeat from Node with CPU status : "+ remoteNodeStatus.getNodeResourceStatus().getAvgCpuUsage() );
    
    LOG.info("JISHA : Received heartbeat from Node with IO status : "+ remoteNodeStatus.getNodeResourceStatus().getAvgIOUsage() );
    
    List<ContainerStatus> continerStatuses = remoteNodeStatus.getContainersStatuses();
    
    for (ContainerStatus c : continerStatuses){

    	LOG.info("JISHA : Received heartbeat from Node with Container status : id = "+ c.getContainerId().toString()+" CPU = "+ c.getCpuUsage()+ " Memory = "+ c.getMemoryUsage());
    }
    ...
    // JISHA
    LOG.info("JISHA : Setting RM NODE Resource status ....." );
    rmNode.setNodeResourceStatus(remoteNodeStatus.getNodeResourceStatus());
    rmNode.setContainersStatus(remoteNodeStatus.getContainersStatuses());
    ...
    // JISHA
    LOG.info("JISHA : Sending status to RM Node");
    
//    this.rmContext.getDispatcher().getEventHandler().handle(
//            new RMNodeStatusEvent(nodeId, remoteNodeStatus.getNodeHealthStatus(),
//                remoteNodeStatus.getContainersStatuses(), 
//                remoteNodeStatus.getKeepAliveApplications(), nodeHeartBeatResponse,
//                remoteNodeStatus.getNodeResourceStatus()));
    this.rmContext.getDispatcher().getEventHandler().handle(
        new RMNodeStatusEvent(nodeId, remoteNodeStatus.getNodeHealthStatus(),
            remoteNodeStatus.getContainersStatuses(), 
            remoteNodeStatus.getKeepAliveApplications(), nodeHeartBeatResponse,
